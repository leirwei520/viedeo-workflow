...
export interface NodeData {
  ...
  mentionParentIds?: string[]; // Parent IDs added via @ mentions in prompt (auto-removed when mention text is deleted)
  ...
  // Auto-generated nodes (three-view, etc.) — hide prompt/controls
  hideControls?: boolean;

  // Batch generation
  generateCount?: number; // Number of images to generate (1, 3, 5) — default 1

  // Storyboard Generator specific
  characterReferenceUrls?: string[];
  assetMentions?: { name: string; url: string }[];
}
...
export interface ContextMenuState {
  ...
  x: number;
  y: number;
  type: 'global' | 'node-connector' | 'node-options' | 'add-nodes' | 'group-connector';
  sourceNodeId?: string; // If 'node-connector' or 'node-options', which node originated the click
  sourceGroupNodeIds?: string[]; // If 'group-connector', the IDs of all group member nodes

...
interface UseChatAgentReturn {
    ...
    sendMessage: (content: string, media?: { type: 'image' | 'video'; url: string; base64?: string }[], canvasContext?: string) => Promise<void>;
    startNewChat: () => void;
...
export function useChatAgent(): UseChatAgentReturn {
    ...
    const refreshSessions = useCallback(async () => {
        ...
        try {
            const response = await authFetch(apiEndpoint('/api/chat/sessions'));
            if (response.ok) {
    ...
    const loadSession = useCallback(async (targetSessionId: string) => {
        ...
        try {
            const response = await authFetch(apiEndpoint(`/chat/sessions/${targetSessionId}`));
            if (!response.ok) {
    ...
    const deleteSession = useCallback(async (targetSessionId: string) => {
        try {
            await authFetch(apiEndpoint(`/chat/sessions/${targetSessionId}`), {
                method: 'DELETE',
            });
    ...
    const sendMessage = useCallback(async (
        ...
        try {
            const messageWithContext = canvasContext
                ? `${content}\n\n[CANVAS_CONTEXT]\n${canvasContext}`
                : content;

            const response = await authFetch(apiEndpoint('/api/chat'), {
                method: 'POST',
                body: JSON.stringify({

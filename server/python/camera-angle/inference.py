...
def load_model() -> bool:
    ...
    try:
        ...
        # Resolve local cache path for offline loading
        from huggingface_hub import try_to_load_from_cache, scan_cache_dir
        import glob
        cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "huggingface", "hub",
                                 "models--Qwen--Qwen-Image-Edit-2509", "snapshots")
        local_base_path = None
        if os.path.exists(cache_dir):
            snapshots = glob.glob(os.path.join(cache_dir, "*"))
            if snapshots:
                local_base_path = snapshots[0]
                print(f"[Camera Angle] Using LOCAL cache for base model: {local_base_path}")
        
        base_model_source = local_base_path if local_base_path else BASE_MODEL_ID
        print(f"[Camera Angle] Loading base model: {base_model_source}")
        ...
        # Load pipeline from local cache (offline) to avoid network issues
        _pipe = QwenImageEditPlusPipeline.from_pretrained(
            base_model_source,
            transformer=transformer,
            torch_dtype=_dtype,
            low_cpu_mem_usage=True,
            local_files_only=True if local_base_path else False,
        )
...
def generate_camera_angle(
    ...
    # With enable_sequential_cpu_offload(), the generator must be on CPU
